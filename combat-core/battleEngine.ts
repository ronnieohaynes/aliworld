import { createBattleMoveState, type BattleMoveState } from '../src/data/battleMoveState'
import { applyMoveCostAfterResolve, consumeTurnFlag } from '../src/data/combatTurnCosts'
import {
...
import {
  ...
  tickDeathClocks,
} from '../src/data/combatSystems'
import type { DeathClock } from '../src/data/combatTypes'
import type { UpcomingMove } from '../src/data/enemyMoves'
import type { PlayerMoveId } from '../src/data/moveIds'
import { getMoveLogDisplayName } from '../src/game/moveHighlightColors'
import {
...
import { isWalkerHeavyTutorialActive } from '../src/data/walkerHeavyTutorial'
import { buildDevSpar, isDevSparNpcId } from '../src/data/devSpar'
import { isGhostCombatId, resolveGhostCombatEntry } from '../src/data/ghostCombat'
import { deriveBuildLoopType, type BuildLoopSkill } from '../src/data/buildName'
import {
  createSeededRng,
  getCombatRng,
  randomCombatSeed,
...
import { EMPTY_NPC_MEMORY } from '../src/store/enemyMemoryStore'
import {
  appendBattleFeedback,
  type BattleFeedbackEvent,
} from '../src/data/battleFeedback'
import {
  applySkillCounterModifiers,
  getSkillCounterRelation,
} from '../src/data/skillCounter'
import { applySkillXpToState as bridgeApplySkillXp } from './xpBridge'
import { getSkillStatBonuses, type SkillId, type SkillsState } from '../src/store/skillStore'
import type { PracticeXpBudget } from '../src/data/practiceDailyReset'
import { resolveEnemyDodgeMove } from '../src/data/dodgeResolver'
import { DEFAULT_EQUIPPED_MOVES } from '../src/data/moveIds'
...
export type BattleState = {
  ...
  /** Server-synced practice XP budget for the day (practice policy only). */
  practiceXpBudget: PracticeXpBudget | null
  /** Combat XP actually applied this practice battle (after daily cap scaling). */
  practiceXpSessionEarned: number
  /** Server-issued or client-local seed — drives all combat RNG for this fight. */
  combatSeed: number
  /** Skills frozen at fight start — all combat math + UI use this, not live store. */
  skillsSnapshot: SkillsState
  /** Build loop derived from skillsSnapshot at INIT — counter matchup stays fixed. */
  buildLoop: BuildLoopSkill | null
  /** When set, enemy AI uses this memory instead of localStorage (scored gym). */
  npcMemoryOverride: import('../src/store/enemyMemoryStore').NpcMemory | null
  ...
  switch (behavior.kind) {
    ...
    case 'dodge': {
      const dodge = resolveEnemyDodgeMove({
        ...
        spdStat: state.npc.stats.spd,
        atkStat: state.npc.stats.atk,
...
function applyPostResolveEffects(
  state: BattleState,
  post: import('../src/data/moves').PostResolveEffects,
): void {
...
function snapshotSkills(skills: SkillsState): SkillsState {
  return {
...
function resolveEnemyIncoming(
  state: BattleState,
  eMove: UpcomingMove,
): import('../src/data/combatSystems').EnemyStrikeResolution {
...
function buildResolveContext(
  ...
  slot?: number,
): import('../src/data/moveResolver').ResolveMoveContext {
  const skills = state.skillsSnapshot
  return {
...
function resolvePlayerMoveBody(
  ...
  eMove: UpcomingMove,
  slot?: number,
): { out: ResolveResult; post: import('../src/data/moves').PostResolveEffects } {
  ...
  let post: import('../src/data/moves').PostResolveEffects = {
    deathClocks: [],
    selfDamage: 0,
    healPlayer: 0,
  }
  ...
  applySkillCounterModifiers(
    out,
    getSkillCounterRelation(state.buildLoop, state.npc.leanSkill),
  )
  ...
  out.incoming = mitigateIncoming(
    ...
    state.skillsSnapshot.defense.level,
    state.battleMove,
    state.skillsSnapshot.attack.level,
  )
...
function showTelegraph(
  ...
  const pick = chooseMove(state.npc.id, state.turn, forced, {
    ...
    memoryOverride: state.npcMemoryOverride,
  })
  return pick
}
...
function enemyActsFirstInResolution(state: BattleState, r?: ResolveResult): boolean {
  const speedBased = !playerActsFirstDespiteSpd(
    ...
    state.npc.stats.spd,
    state.skillsSnapshot.speed.level,
  )
...
function finalizeTurn(state: BattleState, r: ResolveResult): BattleState {
  ...
  const upcomingMove = showTelegraph({
    ...
    npcMemoryOverride: state.npcMemoryOverride,
  })
...
function applySkillXpToState(
  ...
  return bridgeApplySkillXp(state, r, log, state.combatXpPolicy)
}
...
export function createInitialBattleState(
  ...
  const playerHp =
    storedHp != null
      ? Math.max(1, Math.min(playerStats.maxHp, Math.floor(storedHp)))
      : playerStats.maxHp
...
export type { DeathClock, StatusEffectId, MoveCost } from '../src/data/combatTypes'
export { applyStatusToCombat, createEmptyCombatStatus } from '../src/data/combatStatus'
export { MOVES as ENEMY_MOVES } from '../src/data/moveDefinitions'
...
export function applyBattleEndHealing(
  ...
  // Loss persists battle HP — aggression has a real overworld cost.
