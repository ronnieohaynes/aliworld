/** Minimal seeded ghost rows for edge daily-set generation (mirrors src/data/seededGhosts.ts). */

export type EdgeSeededGhost = {
  id: string
  handle: string
  displayName: string
  variantId: string
...
export const AUTHORED_CHAMPION: EdgeSeededGhost = {
...
const ROWS: Omit<EdgeSeededGhost, 'level'>[] = [
...
export const SEEDED_GHOSTS: EdgeSeededGhost[] = ROWS.map((row, i) => ({
  ...row,
  level: LEVELS[i] ?? 10,
}))

export function seededInBand(min: number, max: number): EdgeSeededGhost[] {
  return SEEDED_GHOSTS.filter((g) => g.level >= min && g.level <= max)
}

export function getSeededById(id: string): EdgeSeededGhost | undefined {
