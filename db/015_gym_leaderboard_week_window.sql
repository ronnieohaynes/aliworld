-- Weekly-scoped gym leaderboard window (since + optional until for frozen standings).
...
revoke all on function public.internal_gym_leaderboard_ranked(timestamptz, timestamptz, int) from public;
revoke all on function public.internal_gym_leaderboard_ranked(timestamptz, timestamptz, int) from anon, authenticated;
grant execute on function public.internal_gym_leaderboard_ranked(timestamptz, timestamptz, int) to service_role;

-- Drop legacy 2-arg overload if present.
drop function if exists public.internal_gym_leaderboard_ranked(timestamptz, int);
