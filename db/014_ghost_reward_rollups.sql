-- Patch lifetime progress rollups for ghost full/grind/champion fight tiers.
...
declare
  ...
  fight_tier text := lower(coalesce(new.metadata->>'fightTier', ''));
  ghost_won boolean := public.aw_text_to_bool_safe(new.metadata->>'won');
...
begin
  ...
  if new.event_type = 'battle_end' then
    ...
    if new.event_type = 'ghost_match' then
      if ghost_won or result_text = 'win' then
      ...
      if fight_tier = 'full' then
        c := public.aw_jsonb_counter_inc(c, 'ghost_full_prize_fights_total', 1);
        if ghost_won then
          c := public.aw_jsonb_counter_inc(c, 'ghost_full_prize_wins_total', 1);
        end if;
      elsif fight_tier = 'grind' then
        c := public.aw_jsonb_counter_inc(c, 'ghost_grind_fights_total', 1);
        if ghost_won then
          c := public.aw_jsonb_counter_inc(c, 'ghost_grind_wins_total', 1);
        end if;
      elsif fight_tier = 'champion' then
        c := public.aw_jsonb_counter_inc(c, 'ghost_champion_fights_total', 1);
        if ghost_won then
          c := public.aw_jsonb_counter_inc(c, 'ghost_champion_fights_won_total', 1);
        end if;
      end if;
      if public.aw_text_to_bool_safe(new.metadata->>'fullPrizeWin') then
        c := public.aw_jsonb_counter_inc(c, 'ghost_full_prize_clears_total', 1);
      end if;
      if public.aw_text_to_bool_safe(new.metadata->>'progressAwarded') then
        c := public.aw_jsonb_counter_inc(c, 'ghost_progress_awards_total', 1);
      end if;
